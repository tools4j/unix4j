...
import org.unix4j.CommandBuilder;
...
public class GrepTest {
	...
	private void assertGrep(final MultilineString input, final String expression, final MultilineString expectedOutput, Opt<Grep.ArgName>... options){
		final StringWriter actualOutputStringWriter = new StringWriter();
		final Grep grep = new Grep();
		grep.withArg(Grep.Argument.expression, expression);
		grep.withOpts(options);
		grep.readFrom(new ReaderInput(new StringReader(input.toString())));
		grep.writeTo(new WriterOutput(actualOutputStringWriter));
		grep.execute();
