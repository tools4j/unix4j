...
public class GrepTest {
    final File testResourcesDir = new File("src/test/resources").getAbsoluteFile();
    final ExecutionContextFactory contextFactory = new ExecutionContextFactory() {
        ...
        public ExecutionContext createExecutionContext() {
            ...
            context.setCurrentDirectory(testResourcesDir);
            return context;
        }
    };
    ...
    public void testCountOnRelativeFile(){
        final File testFile = new File(testResourcesDir.getPath() + "/commuting.txt" );
    ...
    public void testCountMultipleAbsolutePaths(){
        final String outputDirPath = testResourcesDir.getPath();
    ...
    public void testCountOnRelativeFilesWithCd(){
        assertEquals2("118: commuting.txt", "7: commuting2.txt", Unix4j.cd(testResourcesDir).grep(Grep.Options.count, "the", "*.txt").sort());
    }
    ...
    public void testLineNumberOnRelativeFiles() throws IOException {
        ...
        final File[] testFiles = new File[] {
                new File(testResourcesDir.getPath() + "/commuting.txt" ),
                new File(testResourcesDir.getPath() + "/commuting2.txt" ),
        };
        assertEquals2(
                "commuting.txt:3:Subject: Commuting for beginners.",
                "commuting2.txt:3:Subject: Commuting for beginners.",
        ...
        final String urlPathPrefix = "file:" + testResourcesDir.getPath().replace('\\', '/');
    ...
    public void testLineNumberWithRegex() {
        final String inputPath = new File(testResourcesDir, "theFileName.theFileExtension").getAbsolutePath();
    ...
    public void testFixedStringWithIgnoreCase() {
        final File file = new File("testFixedStringWithIgnoreCase.text");
        try {
            //given
            ...
            assertEquals2("Hello world", "HELLO WORLD",
                    Unix4j.grep(Grep.Options.F.i, "hEllO wORLd", "testFixedStringWithIgnoreCase.*"));
        } finally {
            file.delete();
        }
    }
